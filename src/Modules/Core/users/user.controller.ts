...
export class UserController {
  ...
  @ApiResponse({ status: 400, description: 'Datos inválidos.' })
  createUserLegacy(
    @Body() createUserLegacyDto: CreateUserLegacyDto,
    @Request() req: any,
  ) {
    const authToken = req.headers.authorization;
    const ipAddress = req.ip || req.connection?.remoteAddress;
  ...
  @ApiOperation({
    ...
    description:
      'Envía un código de verificación al email del usuario para restablecer la contraseña',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Verifica el código de 6 dígitos y establece la nueva contraseña',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint que revisa todas las contraseñas próximas a vencer y envía notificaciones por email a los usuarios correspondientes. Normalmente se ejecuta mediante un cron job.',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        message: {
          type: 'string',
          example:
            'Verificación de contraseñas completada. Se enviaron 5 notificaciones.',
        },
  ...
  async checkPasswordExpiration(
    @Request() req: any,
  ): Promise<PasswordExpirationCheckResult> {
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene una lista de usuarios cuyas contraseñas vencerán en los próximos días especificados. Por defecto busca en los próximos 7 días.',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      items: {
        ...
        properties: {
          ...
          status: {
            type: 'string',
            enum: ['Vencida', 'Vence mañana', 'Crítico', 'Próximo a vencer'],
          },
        },
      },
    },
  ...
  @ApiOperation({
    summary:
      'Obtener lista de usuarios con contraseñas próximas a vencer (7 días por defecto)',
    description:
      'Obtiene una lista de usuarios cuyas contraseñas vencerán en los próximos 7 días.',
  })
  @ApiResponse({
    status: 200,
    description: 'Lista de usuarios con contraseñas próximas a vencer',
  ...
  @ApiResponse({
    status: 200,
    description: 'Jefe directo obtenido exitosamente.',
  })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado.' })
  ...
  @ApiResponse({
    status: 400,
    description: 'Esta asignación crearía un bucle en la jerarquía.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de posibles jefes obtenida exitosamente.',
  })
  ...
  @Get('support-list')
  async getSupportUsersOth() {
    // return 'Hola';
    return this.usersService.findAllSupportUsers();
  }

  @UseGuards(AuthGuard)
  @Get(':id/support-types')
  @ApiOperation({
    summary: 'Obtener tipos de soporte que puede manejar un usuario',
  })
  @ApiResponse({
    status: 200,
    description: 'Tipos de soporte obtenidos exitosamente.',
  })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado.' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Tipos de soporte asignados exitosamente.',
  })
  @ApiResponse({
    status: 400,
    description: 'Algunos tipos de soporte no existen.',
  })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado.' })
  ...
  @ApiOperation({
    summary: 'Obtener usuarios que pueden dar soporte a un tipo específico',
  })
