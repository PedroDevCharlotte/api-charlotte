...
export class TicketNotificationService implements OnModuleInit {
  ...
  private getTicketEmailContext(context: TicketEmailContext) {
    return {
      ...
      content: context.message?.content || context.customMessage || context.ticket.description || '',
  ...
  logoData: this.logoDataLoaded ? this.logoDataCache : this.loadLogoAsDataUri(),
  ...
  private loadLogoAsDataUri(): string | undefined {
    ...
    try {
      ...
      const configured = this.configService.get<string>('CHARLOTTE_LOGO_PATH') || this.configService.get<string>('CHARLOTTE_LOGO_URL');
        ...
        if (configured) {
          if (path.isAbsolute(configured)) {
            ...
            if (!logoPath) {
              ...
              logoPath = path.join(process.cwd(), 'public', 'images', 'logos', fileName);
            }
          } else {
          ...
          // default location
          logoPath = path.join(process.cwd(), 'public', 'images', 'logos', 'Logo azul CCI INTL.png');
        }
      } catch (err) {
        this.logger.debug('Error resolving configured logo path:', (err as Error).message || err);
      }
      ...
      this.logoDataLoaded = true;
      this.logoDataCache = undefined;
      this.logger.debug('Could not load logo as data URI:', err?.message || err);
      return undefined;
    }
  }
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`Logo preloaded: ${v ? 'yes' : 'no'}`);
    } catch (err) {
      this.logger.debug('Error preloading logo:', (err as Error).message || err);
    }
  }
   ...
   */
  private async sendSingleEmail(subject: string, template: string, emailContext: any, recipients: { to?: string[]; cc?: string[] }) {
    const isValidEmail = (e: string) => typeof e === 'string' && /.+@.+\..+/.test(e.trim());
    const toList = (recipients.to || []).map(r => (r || '').toString().trim()).filter(isValidEmail);
    const ccList = (recipients.cc || []).map(r => (r || '').toString().trim()).filter(isValidEmail);
    if (toList.length === 0 && ccList.length === 0) return;

    // Decide primary: prefer assigned, then creator, then first toList
    let primary: string | undefined;
    if (emailContext.ticket?.assignee?.email && toList.includes(emailContext.ticket.assignee.email)) primary = emailContext.ticket.assignee.email;
    if (!primary && emailContext.ticket?.creator?.email && toList.includes(emailContext.ticket.creator.email)) primary = emailContext.ticket.creator.email;
    if (!primary) primary = toList[0] || ccList[0];

    // Build cc as others excluding primary
    const others = Array.from(new Set([...(toList || []), ...(ccList || [])].filter(e => e !== primary)));
  ...
  // Build recipient lists but exclude creator and assignee from participant lists (they are actors)
  const allTo = validTo;
  const allCc = Array.isArray(ccRecipients) ? ccRecipients.map(r => (r || '').toString().trim()).filter(isValidEmail) : [];
  const excludeEmails = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered = allTo.filter(e => !excludeEmails.includes(e));
  const ccFiltered = allCc.filter(e => !excludeEmails.includes(e));

  // Ensure the ticket creator receives a copy (cc) when a ticket is created.
  const creatorEmail = context.ticket.creator?.email;
  if (creatorEmail && isValidEmail(creatorEmail) && !ccFiltered.includes(creatorEmail)) {
    ccFiltered.push(creatorEmail);
  }

  await this.sendSingleEmail(subject, 'ticket-created', emailContext, { to: toFiltered.length ? toFiltered : [context.ticket.creator?.email || context.ticket.assignee?.email].filter(Boolean) as string[], cc: ccFiltered });
  ...
  const exclude = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered2 = toList.map(r => (r || '').toString().trim()).filter(r => r && !exclude.includes(r));
  const ccFiltered2 = ccList.map(r => (r || '').toString().trim()).filter(r => r && !exclude.includes(r));
  await this.sendSingleEmail(subject, 'ticket-updated', emailContext, { to: toFiltered2.length ? toFiltered2 : [context.ticket.creator?.email || context.ticket.assignee?.email].filter(Boolean) as string[], cc: ccFiltered2 });
  ...
  const toList3 = Array.isArray(context.recipients.to) ? context.recipients.to : [];
  const ccList3 = Array.isArray(context.recipients.cc) ? context.recipients.cc : [];
  const exclude3 = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered3 = toList3.map(r => (r || '').toString().trim()).filter(r => r && !exclude3.includes(r));
  const ccFiltered3 = ccList3.map(r => (r || '').toString().trim()).filter(r => r && !exclude3.includes(r));
  await this.sendSingleEmail(subject, 'ticket-status-changed', emailContext, { to: toFiltered3.length ? toFiltered3 : [context.ticket.assignee?.email || context.ticket.creator?.email].filter(Boolean) as string[], cc: ccFiltered3 });
  ...
  const toList4 = Array.isArray(context.recipients.to) ? context.recipients.to : [];
  const ccList4 = Array.isArray(context.recipients.cc) ? context.recipients.cc : [];
  const exclude4 = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered4 = toList4.map(r => (r || '').toString().trim()).filter(r => r && !exclude4.includes(r));
  const ccFiltered4 = ccList4.map(r => (r || '').toString().trim()).filter(r => r && !exclude4.includes(r));
  await this.sendSingleEmail(subject, 'ticket-assigned', emailContext, { to: toFiltered4.length ? toFiltered4 : [context.ticket.assignee?.email || context.ticket.creator?.email].filter(Boolean) as string[], cc: ccFiltered4 });
  ...
  const toList5 = Array.isArray(context.recipients.to) ? context.recipients.to : [];
  const ccList5 = Array.isArray(context.recipients.cc) ? context.recipients.cc : [];
  const exclude5 = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered5 = toList5.map(r => (r || '').toString().trim()).filter(r => r && !exclude5.includes(r));
  const ccFiltered5 = ccList5.map(r => (r || '').toString().trim()).filter(r => r && !exclude5.includes(r));
  await this.sendSingleEmail(subject, 'ticket-commented', emailContext, { to: toFiltered5.length ? toFiltered5 : [context.ticket.assignee?.email || context.ticket.creator?.email].filter(Boolean) as string[], cc: ccFiltered5 });
  ...
  const ccList = Array.isArray(context.recipients.cc) ? context.recipients.cc : [];
  const exclude = [context.ticket.creator?.email, context.ticket.assignee?.email].filter(Boolean) as string[];
  const toFiltered = toList.map(r => (r || '').toString().trim()).filter(r => r && !exclude.includes(r));
  const ccFiltered = ccList.map(r => (r || '').toString().trim()).filter(r => r && !exclude.includes(r));

  await this.sendSingleEmail(subject, 'ticket-closed', emailContext, { to: toFiltered.length ? toFiltered : [context.ticket.assignee?.email || context.ticket.creator?.email].filter(Boolean) as string[], cc: ccFiltered });
